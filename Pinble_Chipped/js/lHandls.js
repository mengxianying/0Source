...
function random_select(amount, Max, Min) {
    var arr = new Array();
    var arr = getArray(amount, Max, Min);
    return arr.sort(asc);
}
...
function InsertShow() {	//重新统计
    ...
    //大乐透 追加玩法 每注3元
    if ($("#playName").val() == 6 && $("#ck_app").attr("checked") == true) {
        document.getElementById("showallmoney").innerHTML = Ps * 3 * beishu;
    }
    else {
    ...
    if ($("#txt_copy").val() == 0 || $("#txt_copy").val() == "" || $("#txt_copy").val() == null) {
        ...
        if (Ps != 0 && Ps * 2 * beishu / nOnlyCopy < 0.2) {
            ...
            $("#lab_money").html(Math.round((Ps * 2 * beishu / nOnlyCopy) * 100) / 100);
        }
        else {
            $("#lab_money").html(Math.round((Ps * 2 * beishu / nOnlyCopy) * 100) / 100);
        }
    ...
    //大乐透 追加玩法 每注3元
    if ($("#playName").val() == 6 && $("#ck_app").attr("checked") == true) {
        document.getElementById("allmoney").value = Ps * 3 * beishu;
    }
    else {
                                        ...
                                        if (result.responseText.split('"')[1].split('"')[0] == 0) {
                                            ...
                                            if ($("input[name='buy']:checked").val() == "track") {
                                                ...
                                                $.ajax({
                                                    ...
                                                    data: "{SerialNum:'" + qNum + "',username:'" + username + "',playName:'" + $("#playName").val() + "',stopCondition:'" + cond + "',issue:'" + issue.substring(issue.length - 1, 0) + "',num:'" + ChoiceNum + "',multiple:'" + doubel.substring(doubel.length - 1, 0) + "',money:'" + InMoney.substring(InMoney.length - 1, 0) + "',message:'1'}",
                                                    dataType: 'json',
                                                    complete: function (result) {
                                                        //result返回的值
                                                        if (result.responseText == 1) {
