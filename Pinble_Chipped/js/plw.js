...
function DisNum() {
    ...
    if ($("input[name='a']:checked").val() == "zixuan") {
        ...
        zhushu = (Ttext[0].length) * (Ttext[1].length) * (Ttext[2].length) * (Ttext[3].length) * (Ttext[4].length);

    }
    // 手动输入号码
    if ($("input[name='a']:checked").val() == "sdsr") {
        for (i = 0; i < str.split('|').length; i++) {
            if (str.split('|')[i].split(',').length == 5) {
                zhushu++;
            }
        }
    }
