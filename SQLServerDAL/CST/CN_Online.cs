...
namespace Pbzx.SQLServerDAL
{
    ...
    /// 数据访问类CN_Online。
    /// </summary>
    public class CN_Online : ICN_Online
    {
        public CN_Online()
        { }
        #region  成员方法
        ...
        public bool Exists(int ID)
        {
            ...
            strSql.Append("select count(1) from CN_Online");
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Pbzx.Model.CN_Online model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into CN_Online(");
        ...
        /// 更新一条数据
        /// </summary>
        public int Update(Pbzx.Model.CN_Online model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update CN_Online set ");
            strSql.Append("Username=@Username,");
            strSql.Append("SoftwareType=@SoftwareType,");
            strSql.Append("InstallType=@InstallType,");
            strSql.Append("ProgramVer=@ProgramVer,");
        ...
        public int Delete(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from CN_Online ");
            strSql.Append(" where ID=@ID ");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = ID;
        ...
        public Pbzx.Model.CN_Online GetModel(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 ID,Username,SoftwareType,InstallType,ProgramVer,HDSN,RN,IP,Port,StartTime,EndTime,LoginMutex,Status,ServiceID,UserIndex,UseType,UseValue from CN_Online ");
            strSql.Append(" where ID=@ID ");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = ID;

            Pbzx.Model.CN_Online model = new Pbzx.Model.CN_Online();
