using System;
using System.Collections.Generic;

using System.Web;
using System.Web.UI;
using System.Web.UI.WebControls;
using System.Data;
using System.Text;

namespace Pbzx.Web.PB_Manage.Platform.match
{
    public partial class UserIntegral : AdminBasic
    {
        DataSet ds_u = new DataSet();
        DataTable IsResult = new DataTable();
        Pbzx.BLL.Challenge_integral get_il = new Pbzx.BLL.Challenge_integral();
        Pbzx.BLL.Challenge_type get_te = new Pbzx.BLL.Challenge_type();
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                BindGridView1("1=1");
            }
        }



        /// <summary>
        /// 序号
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void MyGridView_RowCreated(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowIndex > -1)
            {
                e.Row.Cells[1].Text = Convert.ToString(e.Row.RowIndex + 1 + (AspNetPager1.CurrentPageIndex - 1) * AspNetPager1.PageSize);
            }
        }

        //绑定数据
        public void BindGridView1(string Trem)
        {
            StringBuilder sb = new StringBuilder();
            

            sb.Append(Trem);
            string Searchstr = sb.ToString();
            //发布时间倒序排列
            string order = "I_integral desc";
            int mycount = 0;
            IsResult = get_il.GuestGetBySearchIntegral(Searchstr, "*", order, 50, 3, AspNetPager1.CurrentPageIndex, out mycount);
            this.MyGridView.DataSource = IsResult;
            this.MyGridView.DataKeyNames = new string[] { "I_id" };
            this.MyGridView.DataBind();
            AspNetPagerConfig(mycount);
        }
        protected void AspNetPagerConfig(int tempCount)
        {
            AspNetPager1.PageSize = 50;
            AspNetPager1.RecordCount = tempCount;
            AddCustomText();
        }
        protected void AddCustomText()
        {
            AspNetPager1.CustomInfoHTML = "总计<font color=\"blue\"><b>" + AspNetPager1.RecordCount.ToString() + "</b></font>条&nbsp;";
            AspNetPager1.CustomInfoHTML += "<font color=\"blue\"><b>" + AspNetPager1.CurrentPageIndex.ToString() + "</b>/" + AspNetPager1.PageCount.ToString() + "页</font>";
        }
        protected void AspNetPager1_PageChanged(object sender, EventArgs e)
        {
            BindGridView1("1=1");
        }

       /// <summary>
       /// 删除
       /// </summary>
       /// <param name="sender"></param>
       /// <param name="e"></param>
        protected void MyGridView_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            if (get_il.Delete(Convert.ToInt32(this.MyGridView.DataKeys[e.RowIndex].Value)))
            {
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "deleteScript", "alert('删除成功')", true);
                cb_full.Checked = false;
                BindGridView1("1=1");
            }
            else
            {
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "deleteScript", "alert('删除失败')", true);
            }
        }

        /// <summary>
        /// 多选删除
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void btn_delete_Click(object sender, EventArgs e)
        {
            bool n_state = false;
            for (int i = 0; i < MyGridView.Rows.Count; i++)
            {
                CheckBox cb = (CheckBox)MyGridView.Rows[i].FindControl("cb");
                if (cb.Checked == true)
                {

                    n_state = get_il.Delete(Convert.ToInt32(MyGridView.DataKeys[i].Value));
                }
            }
            if (n_state == true)
            {
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "deleteScript", "alert('删除成功')", true);
                BindGridView1("1=1");
            }
            else
            {
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "deleteScript", "alert('删除失败')", true);
            }
        }
        /// <summary>
        /// 取消
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void btn_cancel_Click(object sender, EventArgs e)
        {
            cb_full.Checked = false;
            for (int i = 0; i <= MyGridView.Rows.Count - 1; i++)
            {
                CheckBox cb = (CheckBox)MyGridView.Rows[i].FindControl("cb");
                cb.Checked = false;
            }
        }

        protected void MyGridView_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            int n_count = this.MyGridView.Rows.Count;
            for (int i = 0; i < n_count; i++)
            {
                //彩种
                Label lab_lott = this.MyGridView.Rows[i].Cells[0].FindControl("lab_lott") as Label;

                //条件名称
                Label lab_name = this.MyGridView.Rows[i].Cells[0].FindControl("lab_name") as Label;

                //行的状态是：正常状态 或者 交替行           
                if (e.Row.RowState == DataControlRowState.Normal || e.Row.RowState == DataControlRowState.Alternate)
                {
                    if (Convert.ToInt32(IsResult.Rows[i]["I_lottid"]) == 3)
                    {
                        lab_lott.Text = "双色球";
                    }
                    if (Convert.ToInt32(IsResult.Rows[i]["I_lottid"]) == 1)
                    {
                        lab_lott.Text = "福彩3D";
                    }
                    if (Convert.ToInt32(IsResult.Rows[i]["I_lottid"]) == 9999)
                    {
                        lab_lott.Text = "排列三";
                    }

                    DataSet ds_name = get_te.GetList("T_state=" + "'" + IsResult.Rows[i]["I_condName"].ToString() + "'");
                    lab_name.Text = ds_name.Tables[0].Rows[0]["T_cond"].ToString();
                }
            }
        }

        /// <summary>
        /// 全选
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void cb_full_CheckedChanged(object sender, EventArgs e)
        {
            for (int i = 0; i <= MyGridView.Rows.Count - 1; i++)
            {
                CheckBox cb = (CheckBox)MyGridView.Rows[i].FindControl("cb");
                if (cb_full.Checked == true)
                {
                    cb.Checked = true;
                }
                else
                {
                    cb.Checked = false;
                }
            }
        }

        /// <summary>
        /// 条件查找
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void btnOK_Click(object sender, EventArgs e)
        {
            string Trem = "";
            if (!string.IsNullOrEmpty(txtName.Text.ToString()))
            {
                Trem = "I_name="+"'"+ txtName.Text.ToString() +"'";
            }
            if (!string.IsNullOrEmpty(txtCondition.Text.ToString()) && Trem != "")
            {
                DataSet dsTrem = get_te.GetList("T_cond=" + "'" + txtCondition.Text.ToString() + "'");
                Trem += " and I_condName=" + "'" + dsTrem.Tables[0].Rows[0]["T_state"].ToString() + "'";
            }
            if (!string.IsNullOrEmpty(txtCondition.Text.ToString()) && Trem == "")
            {
                DataSet dsTrem = get_te.GetList("T_cond=" + "'" + txtCondition.Text.ToString() + "'");
                Trem = "I_condName=" + "'" + dsTrem.Tables[0].Rows[0]["T_state"].ToString() + "'";
            }
            
            BindGridView1(Trem);
        }
        
        protected void btnReset_Click(object sender, EventArgs e)
        {
            Response.Redirect("UserIntegral.aspx");
        }
    }
}