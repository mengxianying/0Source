...
{
    public partial class K3config : AdminBasic
    {
        bool IsLianXu = true;
        bool IsInsert = true;
        int intIssue = 7;//期号长度
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                for (int i = 0; i < this.txtcode.Text.Length; i++)
                {
                    if (Convert.ToInt32(this.txtcode.Text.Substring(i, 1)) > 6)
                    {
                        strErrMsg += "号码输入错误.\\r\\n";
                        break;
                    }
            ...
            else
            {
                if (ViewState["hide"].ToString() == "0")
                {
                    ...
                    if (basicDAL.ExecuteSql(" insert into " + ViewState["tableName"] + "(issue,[date],Code,LastModifytime,OpIp,OpName) values('" + this.txtIssue.Text + "','" + this.txtDate.Text + "','" + arrlist(this.txtcode.Text) + "','" + DateTime.Now + "','" + Request.UserHostAddress + "','" + WebFunc.GetCurrentAdmin() + "')") > 0)
                    {
                        ViewState["hide"] = "0";
                ...
                {
                    if (basicDAL.ExecuteSql("update " + ViewState["tableName"] + " set issue='" + this.txtIssue.Text + "',[date]='" + this.txtDate.Text + "',Code='" + arrlist(this.txtcode.Text) + "',LastModifytime='" + DateTime.Now + "',OpIp='" + Request.UserHostAddress + "',OpName='" + WebFunc.GetCurrentAdmin() + "' where issue='" + ViewState["hide"] + "' ") > 0)
                    {
                        ViewState["hide"] = "0";
                        btnList_Click(new object(), new EventArgs());
                    }
        ...
        private string CalcNexIssue(string issue, string strdate)
        {
            ...
            if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (mynewIssue.Length < 7)
                {
                    mynewIssue = "0" + mynewIssue;
                }
                for (int i = 0; i < tmSZ.Count; i++)
                {
                    if (TimeSpan.Parse(tmSZ[i]) == oldDate.TimeOfDay)//找到该时间所在时间序列位置
                    {
                        ...
                        {
                            if (Request["city"] == "FCSSCData_XinJ")
                            {
                                if (oldDate.TimeOfDay == TimeSpan.Parse("2:00"))
                                {
        ...
        protected void FCSSL_batchSave_code(string text)
        {
            ...
            for (int i = 0; i < textSZ.Length; i++)
            {
                ...
                //号码排序
                haoma = arrlist(textSZ[i].Substring(intIssue).ToString());
        ...
        /// <returns></returns>
        private string arrlist(string str)
        {
            ArrayList arr = new ArrayList();
            for (int j = 0; j < str.Length; j++)
            {
                arr.Add(str.Substring(j, 1));
            }
            ...
            string strNum = arr[0].ToString() + arr[1].ToString() + arr[2].ToString();
            return strNum;
        }
