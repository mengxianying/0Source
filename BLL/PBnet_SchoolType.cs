...
{
    /// <summary>
    /// 业务逻辑类PBnet_SchoolType 的摘要说明。
    /// </summary>
    public class PBnet_SchoolType
    {
        private readonly IPBnet_SchoolType dal = DataAccess.CreatePBnet_SchoolType();
        Pbzx.SQLServerDAL.Basic basicDAL = new Pbzx.SQLServerDAL.Basic("PBnet_SchoolType", "IntNewsTypeID");
        public PBnet_SchoolType()
        { }
        #region  成员方法
        /// <summary>
        /// 是否存在该记录
        ...
        /// 增加一条数据
        /// </summary>
        public bool Add(Pbzx.Model.PBnet_SchoolType model)
        {
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Pbzx.Model.PBnet_SchoolType model)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public Pbzx.Model.PBnet_SchoolType GetModel(int IntNewsTypeID)
        {
        ...
        /// 得到一个对象实体，从缓存中。
        /// </summary>
        public Pbzx.Model.PBnet_SchoolType GetModelByCache(int IntNewsTypeID)
        {

            string CacheKey = "PBnet_SchoolTypeModel-" + IntNewsTypeID;
            ...
            return (Pbzx.Model.PBnet_SchoolType)objModel;
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<Pbzx.Model.PBnet_SchoolType> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Pbzx.Model.PBnet_SchoolType> DataTableToList(DataTable dt)
        {
            List<Pbzx.Model.PBnet_SchoolType> modelList = new List<Pbzx.Model.PBnet_SchoolType>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                Pbzx.Model.PBnet_SchoolType model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Pbzx.Model.PBnet_SchoolType();
        ...
        {
            Pbzx.SQLServerDAL.Basic bac = new Pbzx.SQLServerDAL.Basic("PBnet_SchoolType", "IntNewsTypeID");
        ...
        {
            DataTable dt = GetLisBySql("select * from PBnet_SchoolType where IntTypeFID =" + parentClass + " and  BitIsAuditing=1  order by IntTypeFID ");
        ...
        {
            list.Items.Clear();
            DataTable dt = GetLisBySql("select * from PBnet_SchoolType where IntTypeLevel='" + channelID + "' and BitIsAuditing=1   order by IntTypeFID ");
            foreach (DataRow row in dt.Rows)
            {
                StringBuilder sb = new StringBuilder();
        ...
        {
            return basicDAL.GuestGetBySearch(basicDAL.strPKName, "PBnet_SchoolType", SearchStr, getFileds, OrderStr, PageNum, orderType, CurrentPage, out Counts);
        }


        public DataTable GetListBySort()
        {
            return basicDAL.GetLisBySql("SELECT * FROM PBnet_SchoolType ORDER BY IntSortID ASC");
        }

        public DataTable GetRootList()
        {
            return basicDAL.GetLisBySql("SELECT * FROM PBnet_SchoolType WHERE Depth=0");
        }
        /// <summary>
        /// 获得最新插入的ID.
        /// </summary>
        ...
        /// <returns></returns>
        public Pbzx.Model.PBnet_SchoolType GetModelByTypeName(string newsType)
        {
            List<Pbzx.Model.PBnet_SchoolType> lsNews = GetModelList(" VarTypeName='" + newsType + "'  ");
            if (lsNews.Count > 0)
            {
                return lsNews[0];
            }
        ...
        {
            Pbzx.BLL.PBnet_SchoolType bll = new PBnet_SchoolType();
            List<Pbzx.Model.PBnet_SchoolType> lsNews = bll.GetModelList(" VarTypeName='" + Input.FilterAll(TypeName) + "'  ");
            if (lsNews.Count > 0)
            {
        ...
        {
            Pbzx.Model.PBnet_SchoolType MI = GetModel(moduleid);
        ...
        public string ChannelGetNavigation(int Moduleid, bool IsShowIndexName, string flag)
        {
            ...
            string[] FIDS = ModuleGetContents(Moduleid);
            if (IsShowIndexName)
                FID = "<a  href='" + WebInit.webBaseConfig.WebUrl + "' class='school' target='_self'>" + WebInit.webBaseConfig.WebName + "</a> >> <a href='" + WebInit.webBaseConfig.WebUrl + "School.htm' target='_self' class='school'>软件学院</a>";

            if (!string.IsNullOrEmpty(FIDS[0]))
            {
                for (int i = 0; i < FIDS.Length; i++)
                {
                    Pbzx.Model.PBnet_SchoolType mi = dal.GetModel(int.Parse(FIDS[i]));
                    string ChannelName = "<a  style='cursor:pointer;' href='" + WebInit.webBaseConfig.WebUrl + "School.htm?Type=" + mi.IntNewsTypeID.ToString() + "'  class='school'>" + mi.VarTypeName + "</a>";
                    // string ChannelUrl = m;
        ...
        public DataTable GetTitleListByCount(string strWhere, int count)
        {
            ...
            strSql.Append(" FROM PBnet_SchoolType ");
            if (strWhere.Trim() != "")
            {
        ...
        /// 自己定义根据用户名得到一个对象实体
        /// </summary>
        public Pbzx.Model.PBnet_SchoolType GetModelName(string Name)
        {

            List<Pbzx.Model.PBnet_SchoolType> ls = GetModelList(" IntOrderID='" + Name + "' and BitComment=1 and BitIsAuditing=1");
            if (ls.Count > 0)
            {
                return ls[0];
            }
        ...
        {
            Pbzx.SQLServerDAL.Basic basicDAL1 = new Pbzx.SQLServerDAL.Basic("PBnet_SchoolType", "IntNewsTypeID");
        ...
        /// <param name="y"></param>
        /// <returns></returns>
        private int CompareProductEntry(Pbzx.Model.PBnet_SchoolType x, Pbzx.Model.PBnet_SchoolType y)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public List<Pbzx.Model.PBnet_SchoolType> GetProductListSort(string strWhere)
        {
            List<Pbzx.Model.PBnet_SchoolType> data = GetModelList(strWhere);
            if (data.Count > 1)
            {
                Pbzx.Model.PBnet_SchoolType productM = new Pbzx.Model.PBnet_SchoolType();
                data.Sort(productM);
            }
            return data;
        }
