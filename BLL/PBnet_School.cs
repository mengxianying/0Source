...
{
    /// <summary>
    /// 业务逻辑类PBnet_School 的摘要说明。
    /// </summary>
    public class PBnet_School
    {
        private readonly IPBnet_School dal = DataAccess.CreatePBnet_School();
        Pbzx.SQLServerDAL.Basic basicDAL = new Pbzx.SQLServerDAL.Basic("PBnet_School", "IntID");
        public PBnet_School()
        { }
        #region  成员方法
        /// <summary>
        ...
        /// 增加一条数据
        /// </summary>
        public bool Add(Pbzx.Model.PBnet_School model)
        {
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Pbzx.Model.PBnet_School model)
        {
            if (!model.BitIsPass)
            {
        ...
        /// 得到一个对象实体
        /// </summary>
        public Pbzx.Model.PBnet_School GetModel(int IntID)
        {
        ...
        /// 得到一个对象实体，从缓存中。
        /// </summary>
        public Pbzx.Model.PBnet_School GetModelByCache(int IntID)
        {

            string CacheKey = "PBnet_SchoolModel-" + IntID;
            ...
            return (Pbzx.Model.PBnet_School)objModel;
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<Pbzx.Model.PBnet_School> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Pbzx.Model.PBnet_School> DataTableToList(DataTable dt)
        {
            List<Pbzx.Model.PBnet_School> modelList = new List<Pbzx.Model.PBnet_School>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                Pbzx.Model.PBnet_School model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Pbzx.Model.PBnet_School();
        ...
        {
            return basicDAL.GuestGetBySearch(basicDAL.strPKName, "PBnet_School", SearchStr, getFileds, OrderStr, PageNum, orderType, CurrentPage, out Counts);
        }
        ...
        public int BatchDel(string strID)
        {
            ...
            string sql = "DELETE FROM PBnet_School WHERE IntID IN(" + strID + ")";
            return basicDAL.ExecuteSql(sql);
        ...
        public int BatchUpdate(string strID, string column, bool isTrue)
        {
            ...
            string sql = "update PBnet_School set " + column + "=" + i + " WHERE IntID IN(" + strID + ")";
            return basicDAL.ExecuteSql(sql);
        }
        public DataTable GetTitleListByCount(string strWhere, int count)
        {
            ...
            strSql.Append(" FROM PBnet_School ");
        ...
        {
            Pbzx.SQLServerDAL.Basic basicDAL1 = new Pbzx.SQLServerDAL.Basic("PBnet_School", "IntID");
        ...
        public int ArticleCreate(Pbzx.Model.PBnet_School articleinfo)
        {
            int res = 0;
            try
            {
        ...
        public bool ArticleUpdate(Pbzx.Model.PBnet_School articleinfo)
        {
            ...
            //删除分页数据
            bpa.PaginationDelete(articleinfo.IntID, EModuleType.Schools);
            //更新新闻
            string[] ContentStr = bpa.PaginationContent(articleinfo.NteContent);
            if (ContentStr != null)
            {
        ...
        public bool ArticleDelete(int articleID)
        {
            ...
            try
            {
                ...
                //删除分页数据
                bpa.PaginationDelete(articleID, EModuleType.Schools);
                //删除数据文件
        ...
        public bool ArticleDeleteByModuleID(int moduleid)
        {
            ...
            try
            {
                IList<Pbzx.Model.PBnet_School> ais = ArticleGetByModuleID(moduleid);
                if (ais.Count > 0)
                {
                    foreach (Pbzx.Model.PBnet_School ai in ais)
                    {
        ...
        /// </summary>
        /// <returns></returns>
        public List<Pbzx.Model.PBnet_School> ArticleGetAll()
        {
            try
            {
        ...
        /// <returns></returns>
        public Pbzx.Model.PBnet_School ArticleGetByID(int ArticleID)
        {
            try
            {
        ...
        /// <param name="ModuleID">模块ID</param>
        /// <returns></returns>
        public List<Pbzx.Model.PBnet_School> ArticleGetByModuleID(int ModuleID)
        {
            try
            {
        ...
        /// <returns></returns>
        public List<Pbzx.Model.PBnet_School> ArticleGetByNKey(string NKeyStr)
        {
            try
            {
        ...
        /// <returns></returns>
        public List<Pbzx.Model.PBnet_School> ArticleGetBySearch(string SearchStr, string getFileds, string OrderStr, int PageNum, int orderType, int CurrentPage, out int Counts)
        {
            try
            {
                return DataTableToList(basicDAL.GuestGetBySearch(basicDAL.strPKName, "PBnet_School", SearchStr, getFileds, OrderStr, PageNum, orderType, CurrentPage, out Counts));
            }
            catch (Exception ex)
            {
                Counts = 0;
        ...
        //创建分页数据
        public void ArticleCreatePaginAtion(Pbzx.Model.PBnet_School articleinfo)
        {
            try
            {
                Pbzx.BLL.PBnet_PaginAtion bpa = new Pbzx.BLL.PBnet_PaginAtion();
                Pbzx.BLL.PBnet_SchoolType bc = new PBnet_SchoolType();
                ...
                for (int i = 1; i <= articleinfo.PageNum; i++)
                {
                    ...
                    pai.PhysicPath = BasicProcedure.PhysicalApplicationPath + WebInit.webFolderConfig.SchoolUrl + ArticleUrl + pai.FileName;
                    pai.FilePath = WebInit.webBaseConfig.WebUrl + WebInit.webFolderConfig.SchoolUrl + ArticleUrl + pai.FileName;
                    pai.PageNo = i;
                    pai.Suffix = ".htm";
                    pai.PageType = (int)EModuleType.Schools;
        ...
        public void ArticleDeleteFile(int Articleid)
        {
            try
            {
                ...
                Pbzx.BLL.PBnet_PaginAtion bpa = new Pbzx.BLL.PBnet_PaginAtion();
                IList<Pbzx.Model.PBnet_PaginAtion> pais = bpa.PaginationGet(Articleid, EModuleType.Schools);
                if (pais.Count > 0)
                {
        ...
        public void CreateAllArticleFile()
        {
            try
            {
                List<Pbzx.Model.PBnet_School> ais = ArticleGetAll();
                if (ais.Count > 0)
                {
                    foreach (Pbzx.Model.PBnet_School ai in ais)
                    {
        ...
        public void CreateContentFileByTemplate(int Articleid)
        {
            try
            {
                Pbzx.BLL.PBnet_School newsBLL = new PBnet_School();
                Pbzx.Model.PBnet_School articleInfo = ArticleGetByID(Articleid);
                BTemplate bt = new BTemplate();
                Pbzx.BLL.PBnet_PaginAtion bpa = new Pbzx.BLL.PBnet_PaginAtion();
                Pbzx.BLL.PBnet_SchoolType bc = new PBnet_SchoolType();
                //模板地址
                string TemplateFile = BasicProcedure.PhysicalApplicationPath + "/" + WebInit.webFolderConfig.TemplateUrl + "/" + "SchoolShow.htm";
                //文件地址

                string CreateFile = BasicProcedure.PhysicalApplicationPath + "/" + WebInit.webFolderConfig.SchoolUrl + articleInfo.DatDateAndTime.Year.ToString() + "/" + articleInfo.DatDateAndTime.Month + "/"; //bc.ChannelGetFolder((int)articleInfo.IntShowType)
                ...
                if (articleInfo.PageNum == 1)
                {
                    ...
                    if (bpa.PaginationGet(articleInfo.IntID, EModuleType.Schools).Count < 1)
                    {
                        string[] ContentStr = bpa.PaginationContent(articleInfo.NteContent);
                        if (ContentStr != null)
                        {
                    ...
                    List<Pbzx.Model.PBnet_PaginAtion> pai = bpa.PaginationGet(articleInfo.IntID, EModuleType.Schools);
                    bt.CreateFile(CreateFile + pai[0].FileName, TemplateContent);
                    articleInfo.SavePath = "/" + WebInit.webFolderConfig.SchoolUrl + articleInfo.DatDateAndTime.Year.ToString() + "/" + articleInfo.DatDateAndTime.Month + "/" + pai[0].FileName;
                ...
                else//如果信息有分页
                {
                    ...
                    for (int i = 1; i <= articleInfo.PageNum; i++)
                    {
                        ...
                        articleInfo.SavePath = "/" + WebInit.webFolderConfig.SchoolUrl + articleInfo.DatDateAndTime.Year.ToString() + "/" + articleInfo.DatDateAndTime.Month + "/" + pai[i - 1].FileName;
                        articleInfo.Templet = TemplateFile;
                        newsBLL.Update(articleInfo);
                    }
                }
        ...
        /// <returns></returns>
        public string ReplaceLabel(string TemplateContent, Pbzx.Model.PBnet_School articleInfo)
        {
            ...
            TemplateContent = TemplateContent.Replace("{$CssUrl$}", WebInit.webBaseConfig.WebUrl + WebInit.webFolderConfig.SchoolUrl + "shownews/shownews.css");
