...
{
	/// <summary>
	/// 业务逻辑类PBnet_pdttComment 的摘要说明。
	/// </summary>
	public class PBnet_pdttComment
	{
		private readonly IPBnet_pdttComment dal=DataAccess.CreatePBnet_pdttComment();
		...
		/// 得到一个对象实体，从缓存中。
		/// </summary>
		public Pbzx.Model.PBnet_pdttComment GetModelByCache(int pb_CommentID)
		{
			
			string CacheKey = "PBnet_pdttCommentModel-" + pb_CommentID;
			...
			return (Pbzx.Model.PBnet_pdttComment)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Pbzx.Model.PBnet_pdttComment> GetModelList(string strWhere)
		{
			DataSet ds = dal.GetList(strWhere);
			List<Pbzx.Model.PBnet_pdttComment> modelList = new List<Pbzx.Model.PBnet_pdttComment>();
			int rowsCount = ds.Tables[0].Rows.Count;
			if (rowsCount > 0)
			{
				Pbzx.Model.PBnet_pdttComment model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Pbzx.Model.PBnet_pdttComment();
